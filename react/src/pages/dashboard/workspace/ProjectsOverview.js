        ...
        <AvatarGroup max={5} variant="rounded">
          {process.enabled_teams.length > 0 ? (
            process.enabled_teams.map(team => (
              <TeamAvatar sx={{ height: 25, width: 25 }} key={team.id} team={team} />
            ))
          ) : (
...
const ProjectsOverview = () => {
  ...
  const momentComparator = (a, b) => moment(a).diff(moment(b));
  ...
  const columns = [
    {
      ...
      filterable: false,
      renderCell: params =>
        params.row.is_part_of_publication ? (
          <MenuBook sx={{ mr: 1 }} />
    ...
    },
    {
      ...
      renderCell: params => moment(params.row.created_at).fromNow()
    },
    {
      field: 'status',
      headerName: t('Status'),
      ...
      disableColumnMenu: true,
      filterable: false,
      renderCell: params => <StatusChip t={t} status={params.row.status} />
    },
    {
      field: 'teams',
      headerName: t('Teams'),
    ...
    },
    {
      ...
      disableColumnMenu: true,
      filterable: false,
      renderCell: params => params.row.participation.map(p => <Chip key={p} label={p} />)
    }
  ];
        ...
        <Container maxWidth="lg">
          <AuthGuardSkeleton height="60vh" width="100%">
            <Grid container spacing={3}>
              ...
              >
                <Grid item>
                  <Typography color="textSecondary" variant="overline" data-cy="workspace-title">
                    {t('Workspace')}
                  </Typography>
                  <Typography color="textPrimary" variant="h5" data-cy="welcome-to-user">
                    {t('Welcome', {
            ...
            <Grid container spacing={2}>
              <Grid item xs={6} md={8}>
                <Typography variant="p" sx={{ mt: 3 }}>
                  {`${t(
                    'Set up a new collaborative coproduction process utilizing an already downloaded zip file'
                  )}.`}
                </Typography>
              ...
              <Grid item xs={6} md={4}>
                <Stack direction="row" spacing={0}>
                  ...
                  >
                    {t('Import from file')}
                    <input type="file" accept=".zip" hidden onChange={handleCapture} />
                  </LoadingButton>
                </Stack>
          ...
          <Stack spacing={2} alignItems="center">
            ...
            {showTakeLongMsn && (
              <Alert severity="error">
                The process is taking longer than usual. You can wait here or close the window and
                check back later.
              </Alert>
            )}
          </Stack>
